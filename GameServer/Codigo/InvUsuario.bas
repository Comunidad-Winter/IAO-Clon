...
If UCase$(MapInfo(UserList(UserIndex).Pos.Map).Restringir) = "NEWBIE" Then
    ...
    Select Case UserList(UserIndex).Hogar
        Case eCiudad.cBanderbill 'Vamos a tener que ir por todo el desierto... uff!
            DeDonde = Banderbill
...
Select Case Obj.OBJType
    ...
    Case eOBJType.OtRuna
        ...
        Else
        Call WriteConsoleMsg(UserIndex, "para utilizar este objeto necesitas estar en una ciudad.", FontTypeNames.FONTTYPE_INFO)
               Exit Sub
        End If
        
    Case eOBJType.otNudillos
        If ClasePuedeUsarItem(UserIndex, ObjIndex) Or UserList(UserIndex).flags.Privilegios >= PlayerType.Dios Then
            'Si esta equipado lo quita
            If UserList(UserIndex).Invent.Object(Slot).Equipped Then
                'Quitamos del inv el item
                Call Desequipar(UserIndex, Slot)
...
Select Case Obj.OBJType
    ...
    Case eOBJType.otBarcos
        ...
        If UserList(UserIndex).Stats.ELV < 25 Then
            If UserList(UserIndex).Clase <> eClass.Mercenario Then
                ...
                Else
                    ...
                    Sacrificio.amount = 1 'modificarp or la cantidad a tirar.
                        ...
                        If UserList(UserIndex).Invent.AnilloEqpObjIndex = SACRI Then
                           Call WriteConsoleMsg(UserIndex, "Pierdes tu pendiente, pero tu inventario sigue intacto", FontTypeNames.FONTTYPE_INFO)
