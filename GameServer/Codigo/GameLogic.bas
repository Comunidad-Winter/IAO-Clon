...
Do While Not LegalPos(Pos.map, nPos.X, nPos.Y, PuedeAgua, PuedeTierra)
    ...
    For tY = Pos.Y - LoopC To Pos.Y + LoopC
        For tX = Pos.X - LoopC To Pos.X + LoopC
            ...
            If LegalPos(nPos.map, tX, tY, PuedeAgua, PuedeTierra) And MapData(nPos.map, tX, tY).TileExit.map = 0 Then
                nPos.X = tX
                nPos.Y = tY
                '¿Hay objeto?
...
If InMapBounds(map, X, Y) Then
    ...
    If FoundChar = 1 Then '  ¿Encontro un Usuario?
       ...
       If UserList(TempCharIndex).flags.AdminInvisible = 0 Or UserList(UserIndex).flags.Privilegios And PlayerType.Dios Then
            ...
            If Len(UserList(TempCharIndex).desc) > 1 Then '' Aca esta mal hecho, te va a mostrar primero la ciudad y despues la desc. Osea "Ves a Nhelk <Ciudadano de Ulla> - Soy re capo y te olvidas de esto mira "
                    Stat = "Ves a " & UserList(TempCharIndex).name & Stat & " - " & UserList(TempCharIndex).desc & " <Ciudadano de " & CityNumberToName(UserList(UserIndex).Hogar) & "> "
                Else ''List
                    Stat = "Ves a " & UserList(TempCharIndex).name & Stat & " <Ciudadano de " & CityNumberToName(UserList(UserIndex).Hogar) & "> "
                End If ''Sos bobo?  Te estoy explicando que va a quedar la ciudad antes que el nick... ahhhhh cierto
  ...
  If FoundChar = 2 Then '¿Encontro un NPC?
            ...
            End If
            End If
       ' End If 'mira bldo tiene privilegios aca arriba
...
End If
                'como funciona el sistema ahora? dice le pego primero y la vida algo asi (10/10) si
            End If
            FoundSomething = 1
...
End Sub
Private Function CityNumberToName(ByVal City As eCiudad) As String
    Select Case City
        Case eCiudad.cUllathorpe
            CityNumberToName = "Ullathorpe"
            
        Case eCiudad.cArghal
            CityNumberToName = "Arghal"
        
        Case eCiudad.cBanderbill
            CityNumberToName = "Banderbill"
            
        Case eCiudad.cNix
            CityNumberToName = "Nix"
            
        Case eCiudad.CRinkel
            CityNumberToName = "Rinkel"
    End Select
End Function
...
ItemNoEsDeMapa = ObjData(index).OBJType <> eOBJType.otPuertas And _
            ...
            ObjData(index).OBJType <> eOBJType.otPozo And _
...
Public Function ParticleToLevel(ByVal UserIndex As Integer) As Integer
   ...
   With UserList(UserIndex)
            If .Stats.ELV < 13 Then
                ParticleToLevel = FXIDs.FXMEDITARCHICO
           
            ElseIf .Stats.ELV < 25 Then
                ParticleToLevel = FXIDs.FXMEDITARMEDIANO
           
            ElseIf .Stats.ELV < 35 Then
                ParticleToLevel = FXIDs.FXMEDITARGRANDE
            ElseIf .Stats.ELV < 42 Then
                ParticleToLevel = FXIDs.FXMEDITARXGRANDE
                If Criminal(UserIndex) Then
               ParticleToLevel = FXIDs.FXMEDITARCRIMINAL
               ElseIf .Faccion.ArmadaReal Then
               ParticleToLevel = FXIDs.FXMEDITAARMADA
             ElseIf .Faccion.FuerzasCaos Then
               ParticleToLevel = FXIDs.FXMEDITARCAOS
               End If
            End If
           ...
           ParticleToLevel = FXIDs.FXMEDITARXGRANDE
            ...
            If .Stats.ELV = 50 Then
                ParticleToLevel = FXIDs.FXMEDITARXXGRANDE
            End If
