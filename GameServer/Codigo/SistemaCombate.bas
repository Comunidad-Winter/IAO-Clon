...
Function ModicadorDañoClaseartes(ByVal clase As eClass) As Single
        
    ModicadorDañoClaseartes = ModClase(clase).Dañoartes
...
PoderEvasionEscudo = (UserList(UserIndex).Stats.UserSkills(eSkill.DefensaEscudos) * _
...
End Function
Function PoderAtaqueartes(ByVal UserIndex As Integer) As Long
Dim PoderAtaqueTemp As Long

If UserList(UserIndex).Stats.UserSkills(eSkill.artes) < 31 Then
    PoderAtaqueTemp = (UserList(UserIndex).Stats.UserSkills(eSkill.artes) * _
    ModificadorPoderAtaqueArmas(UserList(UserIndex).clase))
ElseIf UserList(UserIndex).Stats.UserSkills(eSkill.artes) < 61 Then
        PoderAtaqueTemp = ((UserList(UserIndex).Stats.UserSkills(eSkill.artes) + _
        UserList(UserIndex).Stats.UserAtributos(eAtributos.Agilidad)) * _
        ModificadorPoderAtaqueArmas(UserList(UserIndex).clase))
ElseIf UserList(UserIndex).Stats.UserSkills(eSkill.artes) < 91 Then
        PoderAtaqueTemp = ((UserList(UserIndex).Stats.UserSkills(eSkill.artes) + _
        (2 * UserList(UserIndex).Stats.UserAtributos(eAtributos.Agilidad))) * _
        ModificadorPoderAtaqueArmas(UserList(UserIndex).clase))
Else
...
PoderAtaqueartes = (PoderAtaqueTemp + (2.5 * Maximo(CInt(UserList(UserIndex).Stats.ELV) - 12, 0)))
...
Else 'Peleando con puños
    PoderAtaque = PoderAtaqueartes(UserIndex)
End If
...
If UserImpactoNpc Then
    If Arma <> 0 Then
       ...
       End If
    Else
        Call SubirSkill(UserIndex, artes)
    End If
End If
...
Else
    'Pablo (ToxicWaste)
    ModifClase = ModicadorDañoClaseartes(UserList(UserIndex).clase)
    DañoArma = RandomNumber(1, 3) 'Hacemos que sea "tipo" una daga el ataque de artes
    DañoMaxArma = 3
End If
...
Public Sub UserDañoNpc(ByVal UserIndex As Integer, ByVal NpcIndex As Integer)
    ...
    If UserList(UserIndex).flags.Montando = 1 And UserList(UserIndex).Invent.MonturaObjIndex > 0 Then
...
Dim obj As ObjData, defmontura As Integer
...
Call WriteNPCHitUser(UserIndex, Lugar, daño, NpcIndex)
If daño > 145 Then
    ...
    If NpcImpacto(NpcIndex, UserIndex) Then
        With UserList(UserIndex)
            ...
            If UserList(UserIndex).flags.Meditando = False Then
                If UserList(UserIndex).flags.Navegando = 0 And UserList(UserIndex).flags.Montando = 0 Then
...
'Look for NPC
If MapData(AttackPos.map, AttackPos.x, AttackPos.Y).NpcIndex > 0 Then 'Acá muere.
    ...
    Exit Sub
End If ' no, no moría lpm inicia todo de vuelta

    '\/
Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessagePlayWave(SND_SWING, UserList(UserIndex).Pos.x, UserList(UserIndex).Pos.Y))
Call WriteUpdateUserStats(UserIndex)
'eso de play wave qué es? sonido? si osea cuando apretas control y golpea al aire o falla ssuena oel kokokokok ok
...
Else
    PoderAtaque = PoderAtaqueartes(AtacanteIndex)
    ProbExito = Maximo(10, Minimo(90, 50 + _
                ((PoderAtaque - UserPoderEvasion) * 0.4)))
...
End If
  ...
  If UserList(VictimaIndex).flags.Meditando Then
                ...
                Call WriteMeditateToggle(VictimaIndex)
                Call WriteConsoleMsg(VictimaIndex, "Dejas de meditar.", FontTypeNames.FONTTYPE_INFO)
                Call SendData(SendTarget.ToPCArea, VictimaIndex, PrepareMessageDestCharParticle(UserList(VictimaIndex).Char.CharIndex, ParticleToLevel(VictimaIndex)))
        End If
...
If UsuarioImpacto(AtacanteIndex, VictimaIndex) Then
  ...
  If UserList(VictimaIndex).flags.Navegando = 0 And UserList(VictimaIndex).flags.Montando = 0 Then
    ...
    If UserList(AtacanteIndex).clase = eClass.BountyHunter And eClass.Bard Then Call DoHandInmo(AtacanteIndex, VictimaIndex)
...
If UserList(AtacanteIndex).clase = eClass.Bard And BountyHunter Then Call Desarmar(AtacanteIndex, VictimaIndex)
...
UserList(VictimaIndex).Stats.MinHP = UserList(VictimaIndex).Stats.MinHP - daño
    If UserList(VictimaIndex).flags.Meditando Then
            If daño > Fix(UserList(VictimaIndex).Stats.MinHP / 100 * UserList(VictimaIndex).Stats.UserAtributos(eAtributos.Inteligencia) * UserList(VictimaIndex).Stats.UserSkills(eSkill.Meditar) / 100 * 12 / (RandomNumber(0, 5) + 7)) Then
                UserList(VictimaIndex).flags.Meditando = False
                Call WriteMeditateToggle(VictimaIndex)
                Call WriteConsoleMsg(VictimaIndex, "Dejas de meditar.", FontTypeNames.FONTTYPE_INFO)
                Call SendData(SendTarget.ToPCArea, VictimaIndex, PrepareMessageDestCharParticle(UserList(VictimaIndex).Char.CharIndex, ParticleToLevel(VictimaIndex)))
...
Call WriteChatOverHead(SendTarget.ToPCArea, daño, UserList(AtacanteIndex).Char.CharIndex, vbRed)
...
'***************************************************
    ...
    If Criminal(attackerIndex) Then If UserList(attackerIndex).Faccion.ArmadaReal = 1 Then Criminal (attackerIndex)
...
If MapData(UserList(VictimIndex).Pos.map, UserList(VictimIndex).Pos.x, UserList(VictimIndex).Pos.Y).trigger = eTrigger.ZONASEGURA Or _
    ...
    Exit Function
End If
If UserList(attackerIndex).Pos.map = 34 Or UserList(attackerIndex).Pos.map = 20 Or UserList(attackerIndex).Pos.map = 1 Or UserList(attackerIndex).Pos.map = 59 Or UserList(attackerIndex).Pos.map = 60 Or UserList(attackerIndex).Pos.map = 37 Or UserList(attackerIndex).Pos.map = 208 Or UserList(attackerIndex).Pos.map = 150 Or UserList(attackerIndex).Pos.map = 151 Or UserList(attackerIndex).Pos.map = 34 Or UserList(attackerIndex).Pos.map = 217 Or UserList(attackerIndex).Pos.map = 218 Or UserList(attackerIndex).Pos.map = 61 Or UserList(attackerIndex).Pos.map = 111 Or UserList(attackerIndex).Pos.map = 112 Or UserList(attackerIndex).Pos.map = 64 Or UserList(attackerIndex).Pos.map = 62 Or UserList(attackerIndex).Pos.map = 63 Or UserList(attackerIndex).Pos.map = 179 Or UserList(attackerIndex).Pos.map = 194 Or UserList(attackerIndex).Pos.map = 183 Or UserList(attackerIndex).Pos.map = 184 Or UserList(attackerIndex).Pos.map = 185 Then
    Call WriteConsoleMsg(attackerIndex, "No podes pelear aqui.", FontTypeNames.FONTTYPE_WARNING)
    PuedeAtacar = False
    Exit Function
End If
...
'***************************************************
   ...
   'shermie80- Esto ahi que sacarlo por dios.
   ...
   'If ExpaDar <= 0 Then Exit Sub
    ...
    If ExpaDar > 0 Then
        ...
        Else
            ...
            Call WriteConsoleMsg(UserIndex, "Has ganado " & ExpaDar & " puntos de experiencia.", FontTypeNames.FONTTYPE_INFOBOLD)
        End If
