        ...
        'If UserList(UserIndex).Stats.UserSkills(eSkill.Liderazgo) >= 5 Then
            ...
            Else
                ...
                If Not Parties(tInt).NuevoMiembro(UserIndex) Then
                    Call WriteConsoleMsg(UserIndex, "El grupo está lleno, no puedes entrar", FontTypeNames.FONTTYPE_INFOBOLD)
                ...
                Else
                    ...
                    If Not Parties(tInt).HacerLeader(UserIndex) Then
                        Call WriteConsoleMsg(UserIndex, "No puedes hacerte líder.", FontTypeNames.FONTTYPE_INFO)
                    Else
                        Call WriteConsoleMsg(UserIndex, "¡ Te has convertido en líder del grupo !", FontTypeNames.FONTTYPE_INFOBOLD)
                    End If
                End If
           End If
       'Else
...
Dim tInt As Integer
    ...
    If tInt > 0 Then
        If UserList(tInt).PartyIndex > 0 Then
            ...
            Call WriteConsoleMsg(UserIndex, " El fundador decidirá si te acepta en la party", FontTypeNames.FONTTYPE_INFO)
        Else
...
'*************************************************
    ...
    If PI > 0 Then
        ...
        Else
            Call WriteConsoleMsg(User, "¡No eres el líder de tu Party!", FontTypeNames.FONTTYPE_INFO)
            Exit Function
        End If
    Else
...
If UserList(NewMember).PartySolicitud = PI Then
    If Not UserList(NewMember).flags.Muerto = 1 Then
        ...
        End If
    Else
        Call WriteConsoleMsg(leader, "¡Está muerto, no puedes aceptar miembros en ese estado!", FontTypeNames.FONTTYPE_INFO)
        Exit Sub
    End If
Else
